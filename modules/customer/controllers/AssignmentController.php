...
class AssignmentController extends Controller
{
   public $layout = 'customerTemplate';
    ...
    public function actionView($id)
    {
        ...
        $constants = [
            'referencesGeneral' => Reference::TYPE_WALL,
    ...
    public function actionReferenceView($id)
    {
        ...
        if (Yii::$app->request->post()) {
            ...
            if($reference->type == Reference::TYPE_WALL) {
                $path = 'referenceBlockWall';
            }
            if($reference->type == Reference::TYPE_FLOOR) {
                $path = 'referenceBlockFloor';
            }
    ...
    public function actionAddPhotoAjax()
    {
        ...
        if (isset($_POST) AND $_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($idReference = $newPhoto->addImage($file, $assignmentId)) {
                ...
                if ($newPhoto->type == Reference::TYPE_GENERAL) {
                    $imageType = 'image';
                    $commentForm = $this->getCommentForm($idReference);
                }

                if ($newPhoto->type == Reference::TYPE_WALL) {
                    $imageType = 'imageWall';
                    $commentForm = $this->getWallCommentForm($idReference);
                }

                if ($newPhoto->type == Reference::TYPE_FLOOR) {
                    $imageType = 'imageFloor';
                    $commentForm = $this->getFloorCommentForm($idReference);
                }
    ...
    public function actionAddCommentAjax()
    {
        ...
        if ($reference->save()) {
            if ($oldReference->type == Reference::TYPE_GENERAL) {
                $referenceName = 'references';
                $referenceAll = $this->getReferences($oldReference->assignment_id,
                    Reference::TYPE_GENERAL);
            }

            if ($oldReference->type == Reference::TYPE_WALL) {
                $referenceName = 'referencesWall';
                $referenceAll = $this->getReferences($oldReference->assignment_id,
                    Reference::TYPE_WALL);
            }

            if ($oldReference->type == Reference::TYPE_FLOOR) {
                $referenceName = 'referencesFloor';
                $referenceAll = $this->getReferences($oldReference->assignment_id,
                    Reference::TYPE_FLOOR);
            }
